...
def get_parent_text(source_text, en_core, bcd5r):
    ...
    if start_line is not None and mother_father_keyword == mother_father_keywords[0]:
        ...
        parent_name = ' '.join(list(dict.fromkeys(parent_name.split())))
        print(parent_name)
        name_parts = parent_name.split()
        # currently setting mother name_parts 0 because our model detecting other names also person names
        num_words = 0
        if num_words == 2:
        ...
        if num_words == 2:
            # If the author name has only 2 words, consider them as first and last names
            # currently set father and mother key names same because only one block will execute
            mother_first_name = name_parts[0]
            mother_last_name = name_parts[1]
        elif num_words == 0:
            mother_first_name = "Unk"
